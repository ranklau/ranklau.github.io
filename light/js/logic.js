...
var UI = (function() {
	...
	return {
		...
		showinfoframe: function(type, star) {
			...
			$infoframe.find('b').removeClass().addClass(star);
			$infoframe.find('.block').removeClass().addClass('block ' +type);

			var html = [];
			item.tab.forEach(function(entry) {
				var style = item.tab.length==1 ? ' style="width:100%;"' : '';
				html.push('<li '+style+'>' +entry+ '</li>');
			});


			$('#infoframe .tab ul').html(function(){
				return html.join('');
			})

			$('#infoframe .tab li').forEach((function() {
...
$(function() {
	...
	Data.initCallback = function() {
		...
		$('#cal-prev').tap(calPrev);
		$('#cal-next').tap(calNext);

		UI.pageinfoInit();

		myScroll = new iScroll('mainframe',{
			onBeforeScrollStart: function (e) { 
				var target = e.target,
					preserve = ['select', 'input', 'textarea', 'button'],
					tagname; 
				while (target.nodeType != 1) {
					target = target.parentNode; 
				}
				tagname = target.tagName.toLowerCase();
				if (preserve.indexOf(tagname)>-1) return;
